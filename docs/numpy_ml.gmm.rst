...
A Gaussian mixture model (GMM) is a latent variable model commonly used for
...
A GMM assumes that:
    ...
    1. The observed data are generated from a `mixture distribution`_, `P`, made up of `K` mixture components. 

    2. Each mixture component is a multivariate Gaussian with its own mean :math:`\mu`, covariance matrix, :math:`\Sigma`, and mixture weight, :math:`\pi`.
...
The parameters of a GMM model are:
    ...
    - :math:`\theta`, the set of parameters for each of the `K` mixture components. :math:`\theta = \{ \mu_1,
...
.. math::
    p(Z, X \mid \theta) =  \prod_{i=1}^N p(z_i, x_i \mid \theta) = \prod_{i=1}^N \prod_{k=1}^K [\mathcal{N}(x_i \mid \mu_k, \Sigma_k) \pi_k ]^{\mathbb{1}[z_{i} = k]}
...
where 
    ...
    - :math:`\mathbb{1}[z_i = k]` is a binary indicator function returning
